...
function getParamValue(mode, param, numbers, relativeBase): number {
...
module.exports = function createAmp(numbers: { [key: string] : number }) {
  let i = 0;
  let iterations = 0;
  let relativeBase = 0;

  return function intCode(inputSignal: number) {
    while (true) {
