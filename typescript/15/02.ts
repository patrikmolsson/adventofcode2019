...
function newCoordsGivenInput(coords: [number, number], input: Inputs): [number, number] {
...
function paintCoords(coords: [number, number], tile: Tiles) {
  if (!grid[coords[0]]) {
    grid[coords[0]] = {};
  }
...
function moveInDirection(input: Inputs) {
...
function paintCoordsInDirection(input: Inputs, tile: Tiles) {
...
function paintGrid() {
  ...
  const minReducer = (min: number, curr: string) => Math.min(min, Number(curr));
  const maxReducer = (max: number, curr: string) => Math.max(max, Number(curr));
...
function getAllEmptyCoords() {
  let emptyCoords = [];

  for (const rowIndex of Object.keys(grid)) {
    for (const colIndex of Object.keys(grid[rowIndex])) {
      if (!grid[rowIndex]) {
        continue;
      }
      ...
      if (grid[rowIndex][colIndex].tile === Tiles.EMPTY) {
        emptyCoords.push([Number(rowIndex), Number(colIndex)]);
      }
      if (grid[rowIndex][colIndex].tile === Tiles.OX_SYSTEM) {
        emptyCoords.push([Number(rowIndex), Number(colIndex)]);
      }
    }
  }
...
function hasUnexploredTiles(coords: [number, number]) {
  if (!grid[coords[0] - 1]) {
    return true;
  }
  if (!grid[coords[0] + 1]) {
...
function isUnexplored() {
  const emptyCoords = getAllEmptyCoords();

  return emptyCoords.findIndex(c => hasUnexploredTiles(c)) > -1;
}
...
function getMaxSteps() {
  ...
  for (const rowIndex of Object.keys(grid)) {
    for (const colIndex of Object.keys(grid[rowIndex])) {
      ...
      maxSteps = Math.max(grid[rowIndex][colIndex].steps, maxSteps);
    }
  }
...
while (isUnexplored()) {
  ...
  switch (output) {
    ...
    case Output.OX_SYSTEM: {
      moveInDirection(input);
      steps += 1;
      paintCoords(loc, Tiles.OX_SYSTEM);
      if (!hitOxygen) {
        hitOxygen = true;
        resetGrid();
      }
