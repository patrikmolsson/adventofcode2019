...
function changeDirection(directionToTurn: number) {
...
for (let rowI = 0; rowI < rows; rowI += 1) {
  for (let colI = 0; colI < columns; colI += 1) {
    if (!outputArray[rowI]) {
      outputArray[rowI] = '';
    }
    ...
    if (!grid[colI]) {
      ...
      gridValue = grid[colI][rowI] === colors.WHITE ? 'X' : ' ';
    }

    outputArray[rowI] = `${outputArray[rowI]}${gridValue}`;
  }
}
