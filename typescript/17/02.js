...
(function (ASCII_CODES) {
    ...
    ASCII_CODES[ASCII_CODES["ROBOT_DOWN"] = 118] = "ROBOT_DOWN";
    ASCII_CODES[ASCII_CODES["ROBOT_UP"] = 94] = "ROBOT_UP";
    ASCII_CODES[ASCII_CODES["ROBOT_LEFT"] = 60] = "ROBOT_LEFT";
    ASCII_CODES[ASCII_CODES["ROBOT_RIGHT"] = 62] = "ROBOT_RIGHT";
...
function constructAsciiInput() {
    const textRows = inputs.map((input) => input.join(','));
    const asciiRows = textRows.map(row => [...row.split('').map(s => s.charCodeAt(0)), ASCII_CODES.NEWLINE]);
    let inputCodes = [];
    for (const row of asciiRows) {
        inputCodes = [...inputCodes, ...row];
    }
    return inputCodes;
}
const amp = createAmp(numbersMap, constructAsciiInput());
//
// function runWithInput(input: number) {
//   while (true) {
...
//   for (let seq = 0; seq < inputs[rowIndex].length; seq += 1) {
//     const input = inputs[rowIndex][seq];
//
//
//     for (let charIndex = 0; charIndex < input.length; charIndex += 1) {
//       runWithInput(inputs[rowIndex][seq].charCodeAt(charIndex));
//     }
//
//     if (seq < inputs[rowIndex].length - 1) {
//       runWithInput(','.charCodeAt(0));
//     }
//   }
//
//   runWithInput(ASCII_CODES.NEWLINE);
// }
while (true) {
    const output = amp();
    console.log(output);
