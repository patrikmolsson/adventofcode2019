...
var ASCII_CODES;
(function (ASCII_CODES) {
    ASCII_CODES[ASCII_CODES["NEWLINE"] = 10] = "NEWLINE";
    ASCII_CODES[ASCII_CODES["SCAFFOLD"] = 35] = "SCAFFOLD";
    ASCII_CODES[ASCII_CODES["SPACE"] = 46] = "SPACE";
...
while (true) {
    ...
    let charToOutput;
    switch (output) {
        case ASCII_CODES.SCAFFOLD: {
            charToOutput = TILES.SCAFFOLD;
            break;
        }
        case ASCII_CODES.SPACE: {
            charToOutput = TILES.SPACE;
            break;
        }
        default: {
            charToOutput = TILES.ROBOT;
        }
    }
    if (!grid[rowIndex]) {
        grid[rowIndex] = {};
    }
    grid[rowIndex][colIndex] = charToOutput;
    colIndex += 1;
}
function findIntersections() {
    const interserctions = [];
    for (const rowIndex of Object.keys(grid)) {
        for (const colIndex of Object.keys(grid[rowIndex])) {
            if (grid[rowIndex][colIndex] !== TILES.SCAFFOLD) {
                continue;
            }
            if (grid[rowIndex][Number(colIndex) + 1] !== TILES.SCAFFOLD) {
                continue;
            }
            if (grid[rowIndex][Number(colIndex) - 1] !== TILES.SCAFFOLD) {
                continue;
            }
            if (!grid[Number(rowIndex) + 1] || grid[Number(rowIndex) + 1][colIndex] !== TILES.SCAFFOLD) {
                continue;
            }
            if (!grid[Number(rowIndex) - 1] || grid[Number(rowIndex) - 1][colIndex] !== TILES.SCAFFOLD) {
                continue;
            }
            interserctions.push([Number(rowIndex), Number(colIndex)]);
        }
    }
    return interserctions;
}
function alignmentParameters(intersections) {
    return intersections.map(([col, row]) => col * row).reduce((acc, curr) => acc + curr, 0);
}
...
const alignmentParams = alignmentParameters(intersections);
paintGrid();
